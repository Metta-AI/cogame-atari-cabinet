...
# Substitute: atari-cabinet, coworld-atari-cabinet, 4.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-atari-cabinet:ci)
#   SMOKE_SLUG                 game slug                        (atari-cabinet)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/atari-cabinet)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/atari-cabinet-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-atari-cabinet:ci}}"
slug="${SMOKE_SLUG:-atari-cabinet}"
