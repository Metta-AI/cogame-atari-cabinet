...
## added. `SimEvent` never enters `gameHash`, so nothing here can affect
## determinism.
...
proc key*(kind: SimEventKind): string =
  ...
  of Chip: "chip"
  of Breach: "breach"
  of WallDown: "wall_down"
  of Catch: "catch"
  of Release: "release"
  of Concede: "concede"
  of Eliminated: "eliminated"
  of NearMiss: "near_miss"
  of StanceSet: "stance"
  of PhaseChange: "phase"
  of LastStanding: "last_standing"

proc jsonRow*(event: SimEvent): JsonNode =
  ## One JSON-lines row for one tier-2 sim event.
  %*{
    "tick": event.tick,
    "kind": event.kind.key(),
    "cabinet": event.cabinet,
    "alias": (if event.cabinet >= 0: aliasOfCabinet(event.cabinet) else: ""),
    "by": event.by,
    "ball": event.ball,
    "amount": event.amount,
    "detail": event.detail,
