...
# use-after-free fixed in cabinet_replay.nim (emscripten_exit_with_live_runtime)
...
# cabinet_error_len(). Aborting keeps linear memory intact, and the page reads
# cabinet_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "cabinet_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_cabinet_load_replay,_cabinet_frame,_cabinet_input,_cabinet_packet_ptr,_cabinet_packet_len,_cabinet_mismatch_tick,_cabinet_error_ptr,_cabinet_error_len,_cabinet_stage_ptr,_cabinet_stage_len
  """).replace("\n", " ")
)
